...
load('data/V1SP.mat'); load('data/Th1SP.mat'); 
load('data/V2SP.mat'); load('data/Th2SP.mat'); 
...
nx_pop = 57; ny_pop = 50; nz_pop = 21; % population size;
x0 = -4.48:(4.48+4.48)/(nx_pop-1):4.48;
y0 = -0.98:(0.98+0.98)/(ny_pop-1):0.98;
z0 = -0.98:(0.98+0.98)/(nz_pop-1):0.98;
...
[~, mupop, Ypop, sigma] = popGeneratorSP(Zpop, psnr, ind_pop, 2020, type);
pop = [Ypop, mupop, Zpop];
...
for iterSeed = 1:2
    ...
    xx = -4.45 + (4.45+4.45) * rand(3 * n, 1); 
    yy = -0.95 + (0.95+0.95) * rand(3 * n, 1); 
    zz = -0.95 + (0.95+0.95) * rand(3 * n, 1);
    Z = [xx yy zz];
    ind1 = find(insideVT(V1, Th1, Z));
    ind2 = find(insideVT(V2, Th2, Z));
    ind_in = intersect(ind1, ind2);

    [Z, mu, Y] = dataGeneratorRandomSP(Z, sigma, ind_in, iterSeed, type);
